...
timer.Elapsed += async time =>
{
    ...
    try
    {
        var result = await webApi.ParseScaleFromParameterAsync(points, time, RealtimeDataType.Pga);
        if (result.Data != null)
        {
            var data = result.Data.ToArray();
            // 現在の最大震度
    ...
    try
    {
        var result = await webApi.GetEewInfo(time);
        if (result.Data != null)
        {
            // 現在の最大震度
            Console.WriteLine($"*EEW* {result.Data.ReportNum}");
        }
        else if (result.StatusCode == HttpStatusCode.NotFound)
            Console.WriteLine($"404");
        else
            Console.WriteLine($"*EEW* 取得失敗 " + result.StatusCode);
    }
    catch (Exception ex)
    {
        Console.WriteLine($"*EEW* 取得失敗 " + ex);
    }
};
